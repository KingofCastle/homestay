...
<table id="treeTable" class="table table-striped table-bordered table-condensed">
    <tr><th>区域名称</th><th>区域编码</th><th>区域类型</th><!--<th>问问开通状态</th>--><th>车配开通状态</th><th>备注</th><shiro:hasPermission name="sys:area:edit"><th>操作</th></shiro:hasPermission></tr>
    <%--<c:forEach items="${list}" var="area">--%>
        <%--<tr id="${area.id}" pId="${area.parent.id ne requestScope.area.id?area.parent.id:'0'}">--%>
            ...
            <%--<td>--%>
                ...
                <%--</c:if>--%>
                <%--<c:if test="${!fns:endsWith(area.code, '0000')}">--%>
                    <%--${fns:getDictLabel(area.cheOpen, 'open_status',"")}--%>
                <%--</c:if>--%>
            <%--</td>--%>
            <%--<td>${area.remarks}</td>--%>
