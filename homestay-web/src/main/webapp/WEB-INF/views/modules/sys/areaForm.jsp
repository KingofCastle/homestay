...
<body>
	...
	<form:form id="inputForm" modelAttribute="area" action="${ctx}/sys/area/save" method="post" class="form-horizontal">
		...
		<%--<c:if test="${!fns:endsWith(area.code, '0000')}">--%>
			<%--<div class="control-group">--%>
				<%--<label class="control-label">问问开通状态:</label>--%>
				<%--<div class="controls">--%>
					<%--<form:select path="isOpen" htmlEscape="false" class="required">--%>
						<%--<form:options items="${fns:getDictList('open_status')}" itemLabel="label" itemValue="value" htmlEscape="false"/>--%>
					<%--</form:select>--%>
				<%--</div>--%>
			<%--</div>--%>
		<%--</c:if>--%>
		<c:if test="${!fns:endsWith(area.code, '0000')}">
			<div class="control-group">
				<label class="control-label">车配开通状态:</label>
				<div class="controls">
					<form:select path="cheOpen" htmlEscape="false" class="required">
						<form:options items="${fns:getDictList('open_status')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
					</form:select>
				</div>
			</div>
