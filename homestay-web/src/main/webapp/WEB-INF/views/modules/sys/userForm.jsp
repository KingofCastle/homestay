		...
		<div class="control-group">
			...
			<div class="controls">
                <tags:treeselect id="office" name="office.id" value="${user.office.id}" labelName="office.name" labelValue="${user.office.name}"
					title="部门" url="/sys/office/treeData?type=4" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<form:input path="loginName" htmlEscape="false" maxlength="11" class="required phone"/>（格式为手机号码）
			</div>
		</div>
		<div class="control-group">
...
<!-- 		</div> -->
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea path="remarks" htmlEscape="false" rows="3" maxlength="100" class="input-xlarge"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<%--（当归属部门为门店且用户类型为兔波波门店人员时，会同时生成一个可在兔波波助手登录的店长帐号）--%>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">用户角色:</label>
			<div class="controls">
				<form:checkboxes path="roleIdList" items="${allRoles}" itemLabel="name" itemValue="id" htmlEscape="false" /> （用于权限区分）
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">地区限制:</label>
			<div class="controls">
				<form:checkboxes path="areaCodeList" items="${allAreas}" itemLabel="name" itemValue="code" htmlEscape="false" /> （用于权限区分）
			</div>
		</div>
